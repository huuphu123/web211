    ...
    for(var i=0; i < reply.length; i++) {
        reply[i].onclick = function() {
            ...
            if(user_id == -1) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '/checkass_Web/main/news/reply_comment',
                    data: { news_id : news_id, user_id : user_id, content : content, parent_comment : parent_comment},
...
$("#btn-write").on("click", function() {
    ...
    if(user_id == -1) {
        ...
        $.ajax({
            type: 'POST',
            url: '/checkass_Web/main/news/get_date',
            data: { news_id : news_id},
            success:function(response) {
                var date = response;
                $.ajax({
                type: 'POST',
                url: '/checkass_Web/main/news/insert_comment',
                data: { news_id : news_id, user_id : user_id, content : content},
                success:function(response) {
                    var add_reply = 
                    '<li>'+
                    ...
                    '<ul class="comments-list reply-list">' +
                        '<div id="reply_p_' + response + '">' +
                            ...
                            '<br>' +
                            '<button type="button" class="btn btn-info pull-right reply-btn" data-pid=' +  news_id + ' data-uid=' +
                            user_id +
